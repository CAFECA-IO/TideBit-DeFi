...
contract AccountBookHelper {
  function deployBasics(
    address trustedIssuersRegistry,
    address claimTopicsRegistry
  ) external returns (address identityRegistry, address identityStorage, address compliance) {
...
contract AccountBookFactory {
  ...
  function createAccountBook(
    ...
    (address ir, address irs, address comp) = helper.deployBasics(
      trustedIssuersRegistry,
      claimTopicsRegistry
    );

    // Info: (20260223 - Tzuhan) 2. 部署輕量級的 Authority 與 Token Proxies
    TREXImplementationAuthority authority = new TREXImplementationAuthority(
      true,
      address(0),
      address(0)
    );

    authority.addAndUseTREXVersion(
      ITREXImplementationAuthority.Version(1, 0, 0),
    ...
    IdentityRegistry(ir).addAgent(creditToken);
    IdentityRegistry(ir).addAgent(debtToken);
    ...
    IdentityRegistry(ir).transferOwnership(_companyScw);
    IdentityRegistryStorage(irs).transferOwnership(_companyScw);
    ModularCompliance(comp).transferOwnership(_companyScw);
