...
function signUserOpHash(
  ...
  // 1. 建構 clientDataJSON (包含 challenge = userOpHash)
  ...
  // 2. 建構 authenticatorData (模擬)
  const authenticatorData = toHex(Buffer.alloc(37)); // 32 bytes rpIdHash + flags + count

  // 3. 計算簽名訊息: sha256(authenticatorData || sha256(clientDataJSON))
  ...
  // 6. 計算 Challenge 和 Type 在 clientDataJSON 中的位置
  // 這裡簡化處理，直接尋找字串索引 (真實應用需更嚴謹)
...
describe('CompanySCW (Multi-Sig)', function () {
  async function deployFixture() {
    ...
    // 1. 部署 Mock EntryPoint (因為不需要完整 EntryPoint 功能，只需一個地址即可驗證 onlySelf 檢查)
    ...
    // 3. 部署 CompanySCW
    // 注意：建構子參數需符合：address payable _entryPoint, uint256[][] memory _owners, uint256 _threshold
    const companySCW = await viem.deployContract('CompanySCW', [
      mockEntryPoint,
      ownersData,
      BigInt(2), // Threshold = 2
    ]);
  ...
  it('Should PASS with 2 valid sorted signatures (2-of-3)', async function () {
    ...
    const userOp = {
      ...
      maxPriorityFeePerGas: BigInt(10),
      paymasterAndData: '0x' as Hex,
      signature: encodedSignature, // 填入正確編碼的簽名
    };
  ...
  it('Should FAIL with insufficient signatures (1-of-3)', async function () {
    ...
    // 預期失敗：回傳 1 (SIG_VALIDATION_FAILED)
