...
test.describe('TBDTC000003', () => {
  ...
  test('3. 點擊首圖上的開始和信箱聯絡按鈕、白皮書和 AI 報告按鈕下載', async ({page}) => {
    ...
    await expect
      .soft(page.locator('#RedFlagAnalysisDownload'))
  ...
  test('4. 確認按鈕連結跳轉網頁正確。', async ({page}) => {
    ...
    await expect
      .soft(page.locator('#FooterInstagramLink'))
    ...
    await expect
      .soft(page.locator('#FooterUniversityLink'))
      .toHaveAttribute('href', /.*coming-soon/);
    await expect
      .soft(page.locator('#FooterHelpCenterLink'))
      .toHaveAttribute('href', /.*coming-soon/);
