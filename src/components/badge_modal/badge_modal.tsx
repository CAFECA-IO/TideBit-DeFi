...
const BadgeModal = ({modalVisible, modalClickHandler, badgeData}: IBadgeModalProps) => {
  ...
  const isDisplayedModal = modalVisible ? (
    ...
    <div className="fixed inset-0 z-80 flex items-center justify-center overflow-y-auto overflow-x-hidden bg-black/25 outline-none backdrop-blur-sm focus:outline-none">
      <div
        id="BadgeModal"
        className="relative flex h-auto w-9/10 py-6 flex-col space-y-4 rounded-xl border-0 bg-darkGray1 shadow-lg shadow-black/80 outline-none focus:outline-none sm:w-450px"
      >
        ...
        <div className="flex flex-col items-center px-3">
          ...
          <div className="flex flex-col items-center py-3 text-base">
            ...
            <span className="inline-flex items-center text-lightGray4">{displayedReceiveTime}</span>
          </div>
          <div className="inline-flex items-center p-3">
            <span className="mr-2 text-sm text-lightGray4">{t('LEADERBOARD_PAGE.SHARE')}: </span>
            {socialMediaList}
          </div>
        </div>
        ...
        <div className="flex w-full items-center justify-center">
          <RippleButton
            id="BadgeModalDoneButton"
            buttonType="button"
            onClick={modalClickHandler}
            className="w-300px whitespace-nowrap rounded bg-tidebitTheme px-6 py-2 text-base text-white transition-colors duration-300 hover:bg-cyan-600 focus:outline-none disabled:bg-lightGray"
          >
