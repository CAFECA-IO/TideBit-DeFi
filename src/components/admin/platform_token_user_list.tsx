...
export default function PlatformTokenUserList() {
    ...
    const fetchUsers = useCallback(async () => {
        setLoading(true);
        try {
            // Info: (20260128) Using the new service function
            const data = await getPlatformTokenUsers(page, 10);
            setUsers(data.users);
                            ...
                            users.map((user) => (
                                <tr key={user.id} className="bg-slate-950/50 hover:bg-slate-900">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        ...
                                        <span className="ml-1 text-xs text-slate-500">NTD</span>
                                    </td>
                                    <td className="px-6 py-4 text-right">
